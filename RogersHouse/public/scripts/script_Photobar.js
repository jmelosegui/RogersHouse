$(function () {
    ...
    function spreadPictures() {
        ...
        $albums.not($album).stop().animate({ 'bottom': '-200px' }, 300);
        $album.unbind('click');
        ...
        $album.find('.content')
						  ...
						  .animate({ 'rotate': r + 'deg' }, 300);
						      $back.stop().animate({ 'left': '20px' }, 300);
						  });
    }
    ...
    function roomClick() {
            //TODO: Initial Text in the Room Body Content
        $('#roomContent').html($(this).find('.roomdescr').html()).hide().fadeIn("fast");
    }
    ...
    function showImage(nav) {
        ...
        $('<img style=""/>').load(function () {
            ...
            Cufon.replace('.pp_descr', { fontFamily: 'Honey Script' });
            //show navigation
            showNavigation();
    ...
    function showNavigation() {
        $next.stop().animate({ 'right': '5px' }, 100);
        $prev.stop().animate({ 'left': '5px' }, 100);
    }
